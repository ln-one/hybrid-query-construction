...
query expansion. We then evaluate whether DESA improves retrieval quality while
...
Sparse rankings are fused using equal-weight RRF. We compute nDCG@10 and
Recall@20, compare the ordering of query constructions across cutoff depths,
and measure whether each fixed-$L$ result reproduces the ordered Top-20 from
complete-list fusion
\citep{jarvelin-kekalainen-2002-cumulated,cormack-etal-2009-rrf}.
...
Section~\ref{sec:quality-access}. Retrieval quality is measured by nDCG@10 and
...
We evaluate on seven English BEIR collections: SciFact
...
Dense retrieval uses BGE-small-en-v1.5 \citep{xiao-etal-2023-cpack}, with
...
whether Dense and Sparse benefit from different integration rules.
...
Since all three reproduced pipelines are available on FiQA, ArguAna,
Touch\'e-2020, and SCIDOCS, the full-method comparison uses this common
four-dataset subset.

We further compare with the rank-based QuDAR-simple variant, which uses the
same generated references and fuses the Top-1000 OS, ES, OD, and ED rankings
...
corresponding changes in retrieval quality and access depth using
...
The prespecified primary comparisons are DESA against Original and DESA
