...
a pseudo-relevant document to the query for sparse and dense retrieval
...
Following EAHR, we separate complete-list retrieval quality from the channel
depths required to determine that result \citep{zhang2026exact}.

\subsection{Channel-Aware Integration}

Generated evidence can enter retrieval through query text, dense
...
expansion is integrated; DESA instead focuses on how the same evidence should
be constrained differently across the two channels.

QuDAR adaptively fuses four rankings obtained from original and expanded
queries under sparse and dense retrieval \citep{kim-etal-2026-qudar}. DESA
instead modifies the two rankings before fusion: generated evidence expands
Dense semantics while Sparse remains anchored to the original query's lexical
support.
