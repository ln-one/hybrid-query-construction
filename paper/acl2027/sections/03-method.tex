...
\begin{figure}[H]
  ...
  \caption{Design principle of DESA. The same generated passages expand dense
  semantic coverage but remain anchored to the original query's lexical
  support in sparse retrieval.}
...
two retrieval channels to draw additional cues from a shared expansion.
...
We use the complete-list target and bound-based replay of
\citet{zhang2026exact}; DESA changes the query construction, not this evaluation
protocol. Complete dense and sparse rankings are fused by equal-weight RRF
...
favor of the dense channel, until unread documents can no longer change the
membership or order of $T_K$. It records the replay stopping depths
$(L_D,L_S)$ required to determine the complete-list result.
